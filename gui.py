...
def novi_prozor(ime_prezime,pin,broj_racuna,stanje_racuna):
	...
	copy_button=Button(t,text='Copy',image=new_image,command=copy(broj_racuna))
	copy_button.grid(row=0,column=2)

	Label(t,text='Stanje racuna: ').grid(row=1,column=0)
	stanje_racuna=Label(t,text=str(stanje_racuna)+' dinara').grid(row=1,column=1)
	
	uplati_button=Button(t,text="Uplati na racun",command=lambda:uplati_na_racun(broj_racuna,stanje_racuna))
...
def uplati_na_racun(broj_racuna,stanje_racuna):
	t=Toplevel(root)
	...
	uplati=Button(t,text='Izvrsi uplatu',command=lambda:[k.uplata(broj_racuna,stanje_racuna,broj_racuna_primaoca,iznos),tr.dodaj_transakciju(broj_racuna,stanje_racuna,broj_racuna_primaoca,iznos)])
